...
OOH_care_scotland <- master_total %>%
  filter(
    Area == "Scotland",
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive") %>%
  pull(Percentage)
OOH_care_scotland

# For individual GP practices
OOH_care_GP <- master_total %>%
  filter(
...
OOH_care_HSCP_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by HSCP",
  x_lab = "Percentage (%)",
  y_lab = ""
)+
...
#save_plot_with_script_name(OOH_care_HSCP_barchart, height = 12)

################################################################################
##-----------------------------------------------------------------------------#
# Barchart of Scotland average answer by sex #
OOH_care_scotland_by_sex <- master_demographics %>%
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` =="positive",
...
OOH_care_scotland_by_sex_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by sex",
  x_lab = "Percentage (%)",
  y_lab = "Sex", 
...
OOH_care_scotland_by_age <- master_demographics %>%
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` =="positive",
...
OOH_care_scotland_by_age_barchart <- make_barchart_multiple_groups(
  ...
  x_var = Percentage,
  y_var = `Age Band`,
  title = "Percentage of positive ratings for OOH care, by age",
  x_lab = "Percentage (%)", 
  y_lab = "Age Band",
  bar_colour = person_centered_colour
)+
...
OOH_care_scotland_by_SIMD <- master_demographics %>%
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` =="positive",
...
OOH_care_scotland_by_SIMD_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by SIMD",
  x_lab = "Percentage (%)", 
  y_lab = "SIMD decile",
  bar_colour = person_centered_colour
)+
...
OOH_care_scotland_by_urban <- master_demographics %>%
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` =="positive",
...
OOH_care_scotland_by_urban_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by Urban-Rural 8",
  x_lab = "Percentage (%)", 
  y_lab = "",
  bar_colour = person_centered_colour
)+
...
OOH_care_scotland_by_chronic_pain <- master_demographics %>%
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive",
...
OOH_care_scotland_by_chronic_pain_barchart <- make_barchart_multiple_groups(
  ...
  x_var = Percentage,
  y_var = `Chronic Pain`,
  title = "Percentage of positive ratings for OOH care, by Chronic pain",
  x_lab = "Percentage (%)",
...
OOH_care_scotland_by_long_term <- master_demographics %>% 
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive",
...
OOH_care_scotland_by_long_term_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by long term condition",
  x_lab = "Percentage (%)",
...
OOH_care_scotland_by_sexual_orientation <- master_demographics %>% 
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive",
...
OOH_care_scotland_by_sexual_orientation_barchart <- make_barchart_multiple_groups(
  ...
  title = "Percentage of positive ratings for OOH care, by sexual orientation",
  x_lab = "Percentage (%)", 
...
OOH_care_scotland_by_ethnicity <- master_demographics %>% 
  filter(
    Year == survey_year,
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive",
...
OOH_care_scotland_by_ethnicity_barchart <- make_barchart_multiple_groups(
  ...
  x_var = Percentage,
  y_var = Ethnicity,
  title = "Percentage of positive ratings for OOH care, by ethnicity",
...
OOH_care_scotland_timeseries <- master_data_all %>% 
  filter(
    Area == "Scotland",
    `Question Number` %in% question_lookup$OOH_care,
    `Response Option` == "positive",
...
OOH_care_scotland_timeseries_scatter <- make_scatter(
  ...
  x_var = Year,
  y_var = Percentage,
  title = "Timeseries of the percentage of positive ratings for OOH care",
  y_lab = "Percentage (%)",
  x_lab = "Year"
)+
...
#------------## OOH care variation by HSCP  analysis ##--------------------#
#This creates a list of the OOH care % positive broken down by HSCP, cluster and practice name. 
OOH_care_variation_by_GP <- variation_data %>%
  filter(
    `Question Number` == question_OOH_care,
    `Response Option` == "positive",
    !is.na(Percentage)
  )

OOH_care_variation_by_hscp <- OOH_care_variation_by_GP %>%
  group_by(hscp_name) %>%
  summarise(
    num_practices = n(),
...
OOH_care_chosen_hscp <- OOH_care_variation_by_hscp %>%
  arrange(desc(num_clusters), desc(num_practices)) %>%
  slice_head(n = 1) %>% 
  pull(hscp_name)
OOH_care_chosen_hscp

# Get summary stats for the chosen HSCP
OOH_care_variation_by_cluster <- OOH_care_variation_by_GP %>%
  filter(hscp_name == OOH_care_chosen_hscp) %>% 
...
top_range <- OOH_care_variation_by_cluster %>%
  arrange(desc(range_pct)) %>%
  slice_head(n = 5)

OOH_care_cluster_variation_tails <- top_range %>% 
  pull(hscp_gpcl_name)
OOH_care_cluster_variation_tails

#Plot a combinition of the most varied clusters 
OOH_care_variation_by_GP_Cluster_and_HSCP_plot_data <- OOH_care_variation_by_GP %>%
  ungroup() %>% 
  filter(
    hscp_name == OOH_care_chosen_hscp,
    hscp_gpcl_name %in% OOH_care_cluster_variation_tails
  ) %>%
  mutate(
    cluster_label = paste(
      "GP Cluster",
...
OOH_care_variation_by_GP_Cluster_and_HSCP_plot <-
  make_jitter_plot(
    data = OOH_care_variation_by_GP_Cluster_and_HSCP_plot_data %>%
      filter(
        hscp_name == OOH_care_chosen_hscp,
        hscp_gpcl_name %in% OOH_care_cluster_variation_tails
      ) %>%
    ...
    title = "Variation of the positive OOH care rating for practices within a HSCP, grouped by GP Clusters",
    x_lab = "",
    y_lab = "Percentage of respondents rating their OOH care positively (%)"
  ) +
  scale_shape_manual(
